...
Future<String> deleteDelivery(int masterID) async {
  try {
    SecureStorage secureStorage = SecureStorage();
    String userID = await secureStorage.readSecureData("userID");
    String companyID = await secureStorage.readSecureData("companyID");
    ...
    String url = "${APIConstants.baseUrl}/api/Area/DeleteArea";
    final response = await http.post(Uri.parse(url),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
...
Future<List<AreaDropdownModel>> getAreasDropdown() async {
  try {
    SecureStorage secureStorage = SecureStorage();
    String userID = await secureStorage.readSecureData("userID");
    String companyID = await secureStorage.readSecureData("companyID");
    ...
    String url = "${APIConstants.baseUrl}/api/Dropdown/AreasDropdown";
    ...
    if (responseBody['message'] == "" && responseBody['success'] == true) {
      List<dynamic> data = responseBody['data'];
      return data.map((item) => AreaDropdownModel.fromJson(item)).toList();
    } else {
      throw Exception("${responseBody['message']}");
    }
  } catch (e) {
